class NeuralNet:
    ...
    def __init__(self, X_data, Y_data, layer_sizes, num_iters, eta, act, g, lam, mini):
        import numpy as np
        ...
        if self.g == "reg":
            ...
            for i in range(layers,0,-1):
                ...
                #Propage error backward
                    ...
                    dZ = np.multiply(dA, d_act(values["a" + str(i)]))
                #If at input layer
                if i==1:
                    grads["weight" + str(i)] = 1/m*np.dot(dZ, self.X_train.T)
    ...
    def model(self):
        ...
        if min == True:
            ...
            for i in range(self.num_iters):
                values = NeuralNet.feed_forward(self, params)
                grads = NeuralNet.backprop(self, params, values)
                params = NeuralNet.update_params(self, params, grads)
